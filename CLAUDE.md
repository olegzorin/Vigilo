...
```
foundation ← { aws, messaging } ← core ← [worker, api]
foundation ← mcp
```
...
- **core**: DAO layer (MyBatis), services, domain objects, serialized lambda event/execution contracts, and system/integration services. worker and api depend on this.
...
- `dev.olegz.vf.core.dao` - DAO interfaces with MyBatis mappers in `dao.mapper`
- `dev.olegz.vf.core.service` - Business logic services
- `dev.olegz.vf.core.domain` - Domain/entity objects
...
- Spring XML configs in `core/src/main/resources/vf/`
...
- AWS SDK v2 (EC2, ECS, S3, SQS, SNS, etc.)
