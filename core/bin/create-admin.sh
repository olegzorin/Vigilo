...
# Thin wrapper over the `create-admin` Maven profile (dev.olegz.vf.registry.tool.CreateAdminTool).
#
# Usage:
...
MVN_ARGS=(-q -f "$ROOT/pom.xml" -pl registry -Pcreate-admin compile exec:java)
if [ "$#" -gt 0 ]; then
    joined="$1"; shift
