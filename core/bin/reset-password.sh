...
# Thin wrapper over the `reset-password` Maven profile (dev.olegz.vf.registry.tool.ResetPasswordTool).
#
# Usage:
...
# Scope the reactor to `registry` only (no -am): `exec:java` is a direct CLI goal, so it runs on every
# module in the reactor, and only `registry` defines the profile's mainClass. `compile` in the same
...
MVN_ARGS=(-q -f "$ROOT/pom.xml" -pl registry -Preset-password compile exec:java)
if [ "$#" -gt 0 ]; then
    joined="$1"; shift
